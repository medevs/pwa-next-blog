    ...
    <>
      <section className='max-w-[702px]'>
        ...
        <div className="flex flex-wrap">
          <div key='all' className="w-1/3 sm:w-1/3 md:w-1/4 lg:w-1/5 xl:w-1/5">
            <Tag className='text-xs m-2' onClick={() => handleTagChange(null)}>~All</Tag>
          </div>
          {allTags.map((tag) =>
            <div key={tag} className="w-1/3 sm:w-1/3 md:w-1/4 lg:w-1/5 xl:w-1/5">
              <Tag className='text-xs m-1' onClick={() => handleTagChange(tag)}>#{tag}</Tag>
            </div>
          )}
        </div>
      </section>
